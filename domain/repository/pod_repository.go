...
func (u *PodRepository) InitTable() error {
	return u.mysqlDb.Set("gorm:table_options", "ROW_FORMAT=DYNAMIC").CreateTable(&model.Pod{}, &model.PodEnv{}, &model.PodPort{}).Error
}

//根据ID查找Pod信息
func (u *PodRepository) FindPodByID(podID int64) (pod *model.Pod, err error) {
	pod = &model.Pod{}
	return pod, u.mysqlDb.Preload("PodPort").Preload("PodEnv").First(pod, podID).Error
}
